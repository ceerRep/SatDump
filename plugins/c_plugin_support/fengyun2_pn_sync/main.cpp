...
plugin_error_t plugin_acquire(plugin_acquire_info_t acquire_info, plugin_acquire_result_t *acquire_result)
{
    ...
    pdata->counter = (volatile long long *)acquire_info.acquire_space_by_name(
        pdata->acquire_info.plugin_interface_data, "fengyun2_pn_sync_symbol_counter", sizeof(long long));
    *(pdata->counter) = 0;
...
#define PUSHBIT(x, val)                                                                                                \
    do                                                                                                                 \
    {                                                                                                                  \
        uint8_t bit_to_write = x ^ last_write_bit;                                                                     \
        last_write_bit = !!bit_to_write;                                                                               \
        pdata->buffer[output_buffer_bit_index] = bit_to_write ? val : -val;                                            \
